...
static inline QString certificateHash(const Certificate &cert)
{
    if (cert.isNull()) {
        return QString();
    } else {
...
protected:
    static void __logHook(void *const global_data, const unsigned flags, const char *const format, va_list args);

    static void __slotEventHook(void *const global_data);

    static PKCS11H_BOOL __tokenPromptHook(void *const              global_data,
                                          void *const              user_data,
                                          const pkcs11h_token_id_t token,
                                          const unsigned           retry);

    static PKCS11H_BOOL __pinPromptHook(void *const              global_data,
                                        void *const              user_data,
                                        const pkcs11h_token_id_t token,
                                        const unsigned           retry,
                                        char *const              pin,
    ...
    void _logHook(const unsigned flags, const char *const format, va_list args);

    void _slotEventHook();

    PKCS11H_BOOL
    _tokenPromptHook(void *const user_data, const pkcs11h_token_id_t token);

    PKCS11H_BOOL
    _pinPromptHook(void *const user_data, const pkcs11h_token_id_t token, char *const pin, const size_t pin_max);
};
...
private:
    ...
    public:
        pkcs11KeyStoreItem(const int id, const pkcs11h_token_id_t token_id)
        {
            _id = id;
            ;
        ...
        void registerCertificates(const QList<Certificate> &certs)
        {
...
public:
    ...
    bool _tokenPrompt(void *const user_data, const pkcs11h_token_id_t token_id);

    bool _pinPrompt(void *const user_data, const pkcs11h_token_id_t token_id, SecureArray &pin);
...
private:
    pkcs11KeyStoreItem *_registerTokenId(const pkcs11h_token_id_t token_id);
...
public:
        ...
        : RSAContext(p)
    {
        ...
        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::pkcs11RSAContext1 - entry"), Logger::Debug);
        ...
        if ((rv = pkcs11h_certificate_duplicateCertificateId(&_pkcs11h_certificate_id, pkcs11h_certificate_id)) !=
            CKR_OK) {
            throw pkcs11Exception(rv, QStringLiteral("Memory error"));
        }

        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::pkcs11RSAContext1 - return"), Logger::Debug);
    }

    pkcs11RSAContext(const pkcs11RSAContext &from)
        : RSAContext(from.provider())
    {
        CK_RV rv;

        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::pkcs11RSAContextC - entry"), Logger::Debug);
        ...
        if ((rv = pkcs11h_certificate_duplicateCertificateId(&_pkcs11h_certificate_id, from._pkcs11h_certificate_id)) !=
            CKR_OK) {
            throw pkcs11Exception(rv, QStringLiteral("Memory error"));
        }

        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::pkcs11RSAContextC - return"), Logger::Debug);
    }

    ~pkcs11RSAContext() override
    {
        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::~pkcs11RSAContext - entry"), Logger::Debug);
        ...
        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::~pkcs11RSAContext - return"), Logger::Debug);
    }
...
public:
    ...
    {
        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::convertToPublic - entry"), Logger::Debug);
        ...
        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::convertToPublic - return"), Logger::Debug);
    }
    ...
    bool decrypt(const SecureArray &in, SecureArray *out, EncryptionAlgorithm alg) override
    {
        ...
        try {
            ...
            if ((rv = pkcs11h_certificate_lockSession(_pkcs11h_certificate)) != CKR_OK) {
                throw pkcs11Exception(rv, QStringLiteral("Cannot lock session"));
            }
            session_locked = true;

            if ((rv = pkcs11h_certificate_decryptAny(_pkcs11h_certificate,
                                                     mech,
            ...
            if ((rv = pkcs11h_certificate_releaseSession(_pkcs11h_certificate)) != CKR_OK) {
            ...
            if (session_locked) {
                pkcs11h_certificate_releaseSession(_pkcs11h_certificate);
                session_locked = false;
            }
    ...
    QByteArray endSign() override
    {
        ...
        try {
            ...
            if ((rv = pkcs11h_certificate_signAny(_pkcs11h_certificate,
                                                  CKM_RSA_PKCS,
...
public:
    ...
    bool _isTokenAvailable() const
    {
        bool ret;

        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::_ensureTokenAvailable - entry"), Logger::Debug);
    ...
    bool _ensureTokenAccess()
    {
        bool ret;

        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::_ensureTokenAccess - entry"), Logger::Debug);
...
private:
    ...
    void _ensureCertificate()
    {
        ...
        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::_ensureCertificate - entry"), Logger::Debug);

        if (_pkcs11h_certificate == nullptr) {
            if ((rv = pkcs11h_certificate_create(_pkcs11h_certificate_id,
                                                 &_serialized,
                                                 PKCS11H_PROMPT_MASK_ALLOW_ALL,
                                                 PKCS11H_PIN_CACHE_INFINITE,
                                                 &_pkcs11h_certificate)) != CKR_OK) {
                throw pkcs11Exception(rv, QStringLiteral("Cannot create low-level certificate"));
            }
        }

        QCA_logTextMessage(QStringLiteral("pkcs11RSAContext::_ensureCertificate - return"), Logger::Debug);
    }
};
...
public:
        ...
        : KeyStoreEntryContext(p)
    {
        ...
        _key       = key;
        _storeId = storeId, _id = certificateHash(key.certificateChain().primary());
        _serialized = serialized;
        _storeName  = storeName;
        _name       = name;
    }

    pkcs11KeyStoreEntryContext(const pkcs11KeyStoreEntryContext &from)
...
public:
    ...
    {
        return static_cast<pkcs11RSAContext *>(static_cast<PKeyContext *>(_key.privateKey().context())->key())
            ->_isTokenAvailable();
    }
...
{
private:
    static int _pkcs11h_crypto_qca_initialize(void *const global_data)
    {
    ...
    static int _pkcs11h_crypto_qca_uninitialize(void *const global_data)
    {
        Q_UNUSED(global_data);
    ...
    static int _pkcs11h_crypto_qca_certificate_get_expiration(void *const                global_data,
    ...
    static int _pkcs11h_crypto_qca_certificate_get_dn(void *const                global_data,
                                                      const unsigned char *const blob,
                                                      const size_t               blob_size,
                                                      char *const                dn,
                                                      const size_t               dn_max)
    ...
    static int _pkcs11h_crypto_qca_certificate_is_issuer(void *const                global_data,
                                                         ...
                                                         const size_t               cert_blob_size)
    {
        ...
        Certificate signer = Certificate::fromDER(QByteArray((char *)signer_blob, signer_blob_size));

        Certificate cert = Certificate::fromDER(QByteArray((char *)cert_blob, cert_blob_size));
...
pkcs11h_engine_crypto_t pkcs11QCACrypto::crypto = {nullptr,
...
// pkcs11KeyStoreListContext
//----------------------------------------------------------------------------
pkcs11KeyStoreListContext::pkcs11KeyStoreListContext(Provider *p)
    : KeyStoreListContext(p)
{
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::pkcs11KeyStoreListContext - return"), Logger::Debug);
}

pkcs11KeyStoreListContext::~pkcs11KeyStoreListContext()
{
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::~pkcs11KeyStoreListContext - entry"), Logger::Debug);
...
Provider::Context *pkcs11KeyStoreListContext::clone() const
{
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::clone - entry/return"), Logger::Debug);
    return nullptr;
}

void pkcs11KeyStoreListContext::start()
{
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::start - entry"), Logger::Debug);

    QMetaObject::invokeMethod(this, "doReady", Qt::QueuedConnection);

    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::start - return"), Logger::Debug);
}
    ...
    try {
        if (serialized.startsWith(QLatin1String("qca-pkcs11/"))) {
            ...
            entry = _keyStoreEntryByCertificateId(
                certificate_id, has_private, chain, friendlyNames[certificateHash(chain.primary())]);
        }
    } catch (const pkcs11Exception &e) {
        s_keyStoreList->_emit_diagnosticText(
...
KeyStore::Type pkcs11KeyStoreListContext::type(int id) const
{
    Q_UNUSED(id);
...
QString pkcs11KeyStoreListContext::storeId(int id) const
{
    QString ret;
...
QString pkcs11KeyStoreListContext::name(int id) const
{
    QString ret;
...
QList<KeyStoreEntry::Type> pkcs11KeyStoreListContext::entryTypes(int id) const
{
    Q_UNUSED(id);
...
QList<int> pkcs11KeyStoreListContext::keyStores()
{
    pkcs11h_token_id_list_t tokens = nullptr;
    QList<int>              out;

    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::keyStores - entry"), Logger::Debug);
    ...
    try {
         ...
         */
        if ((rv = pkcs11h_token_enumTokenIds(PKCS11H_ENUM_METHOD_CACHE_EXIST, &tokens)) != CKR_OK) {
        ...
        for (pkcs11h_token_id_list_t entry = tokens; entry != nullptr; entry = entry->next) {
...
QList<KeyStoreEntryContext *> pkcs11KeyStoreListContext::entryList(int id)
{
    ...
    try {
        ...
        if (_storesById.contains(id)) {
            ...
            if ((rv = pkcs11h_certificate_enumTokenCertificateIds(entry->tokenId(),
            ...
            for (current = certs; current != nullptr; current = current->next) {
                if (current->certificate_id->certificate_blob_size > 0) {
                    listCerts += Certificate::fromDER(QByteArray((char *)current->certificate_id->certificate_blob,
                                                                 current->certificate_id->certificate_blob_size));
                }
            }

            for (current = issuers; current != nullptr; current = current->next) {
                if (current->certificate_id->certificate_blob_size > 0) {
                    listIssuers += Certificate::fromDER(QByteArray((char *)current->certificate_id->certificate_blob,
                                                                   current->certificate_id->certificate_blob_size));
                }
            }
            ...
            for (i = 0, current = issuers; current != nullptr; i++, current = current->next) {
                try {
                    if (listIssuers[i].isNull()) {
                        throw pkcs11Exception(CKR_ARGUMENTS_BAD, QStringLiteral("Invalid certificate"));
                    }

                    if (listIssuers[i].isSelfSigned() && dynamic_cast<pkcs11Provider *>(provider())->_allowLoadRootCA) {
            ...
            for (i = 0, current = certs; current != nullptr; i++, current = current->next) {
                try {
...
bool pkcs11KeyStoreListContext::_tokenPrompt(void *const user_data, const pkcs11h_token_id_t token_id)
{
    KeyStoreEntry         entry;
...
bool pkcs11KeyStoreListContext::_pinPrompt(void *const user_data, const pkcs11h_token_id_t token_id, SecureArray &pin)
{
    KeyStoreEntry         entry;
    KeyStoreEntryContext *context = nullptr;
    QString               storeId, storeName;
    ...
    PasswordAsker asker;
    asker.ask(Event::StylePIN, KeyStoreInfo(KeyStore::SmartCard, storeId, storeName), entry, context);
    asker.waitForResponse();
    if (asker.accepted()) {
        ret = true;
        pin = asker.password();
...
void pkcs11KeyStoreListContext::_emit_diagnosticText(const QString &t)
{
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::_emit_diagnosticText - return"), Logger::Debug);
}

void pkcs11KeyStoreListContext::doReady()
{
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::doReady - entry"), Logger::Debug);

    emit busyEnd();

    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::doReady - return"), Logger::Debug);
}

void pkcs11KeyStoreListContext::doUpdated()
{
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::doUpdated - entry"), Logger::Debug);

    emit updated();

    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::doUpdated - return"), Logger::Debug);
}
...
pkcs11KeyStoreListContext::_registerTokenId(const pkcs11h_token_id_t token_id)
{
    ...
    while (i != _stores.end() && !pkcs11h_token_sameTokenId(token_id, (*i)->tokenId())) {
        i++;
    }
...
void pkcs11KeyStoreListContext::_clearStores()
{
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::_clearStores - entry"), Logger::Debug);
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11KeyStoreListContext::_clearStores - return"), Logger::Debug);
}
                                                         ...
                                                         const QString                 &_description) const
{
    ...
    QString serialized = _serializeCertificate(certificate_id, chain, has_private);

    QString            description = _description;
    const Certificate &cert        = chain.primary();
    if (description.isEmpty()) {
        description = cert.subjectInfoOrdered().toString() + QStringLiteral(" by ") +
            cert.issuerInfo().value(CommonName, QStringLiteral("Unknown"));
    }

    if (has_private) {
        pkcs11RSAContext *rsakey =
            new pkcs11RSAContext(provider(), certificate_id, serialized, cert.subjectPublicKey().toRSA());
        ...
        privkey.change(pkc);
        KeyBundle key;
        key.setCertificateChainAndKey(chain, privkey);

        entry = new pkcs11KeyStoreEntryContext(key,
        ...
        entry = new pkcs11KeyStoreEntryContext(cert,
                                               _tokenId2storeId(certificate_id->token_id),
                                               serialized,
                                               QString::fromLatin1(certificate_id->token_id->label),
                                               description,
...
QString pkcs11KeyStoreListContext::_tokenId2storeId(const pkcs11h_token_id_t token_id) const
{
    QString storeId;
    size_t  len;
    ...
    if (pkcs11h_token_serializeTokenId(buf.data(), &len, token_id) != CKR_OK) {
        throw pkcs11Exception(CKR_FUNCTION_FAILED, QStringLiteral("Cannot serialize token id"));
    }
                                                         ...
                                                         const bool                     has_private) const
{
    ...
    if (pkcs11h_certificate_serializeCertificateId(nullptr, &len, certificate_id) != CKR_OK) {
    ...
    if (pkcs11h_certificate_serializeCertificateId(buf.data(), &len, certificate_id) != CKR_OK) {
        throw pkcs11Exception(CKR_FUNCTION_FAILED, QStringLiteral("Cannot serialize certificate id"));
    }
                                                        ...
                                                        CertificateChain               &chain) const
{
    ...
    try {
        ...
        *p_has_private    = false;
        ...
        if ((rv = pkcs11h_certificate_deserializeCertificateId(&certificate_id,
                                                               myPrintable(_unescapeString(list[n++])))) != CKR_OK) {
        ...
        *p_has_private = list[n++].toInt() != 0;
        ...
        if ((rv = pkcs11h_certificate_setCertificateIdCertificateBlob(certificate_id,
                                                                      (const unsigned char *)endCertificateBytes.data(),
                                                                      (size_t)endCertificateBytes.size())) != CKR_OK) {
            throw pkcs11Exception(rv, QStringLiteral("Invalid serialization"));
        }
...
QString pkcs11KeyStoreListContext::_escapeString(const QString &from) const
{
    QString to;
...
QString pkcs11KeyStoreListContext::_unescapeString(const QString &from) const
{
    QString to;
...
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::pkcs11Provider - entry"), Logger::Debug);
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::pkcs11Provider - return"), Logger::Debug);
}

pkcs11Provider::~pkcs11Provider()
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::~pkcs11Provider - entry/return"), Logger::Debug);
}

int pkcs11Provider::qcaVersion() const
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::qcaVersion - entry/return"), Logger::Debug);
...
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::init - entry"), Logger::Debug);
    ...
    try {
        ...
        if ((rv = pkcs11h_setLogHook(__logHook, this)) != CKR_OK) {
            throw pkcs11Exception(rv, QStringLiteral("Cannot set hook"));
        }

        pkcs11h_setLogLevel(0);

        if ((rv = pkcs11h_setTokenPromptHook(__tokenPromptHook, this)) != CKR_OK) {
            throw pkcs11Exception(rv, QStringLiteral("Cannot set hook"));
        }

        if ((rv = pkcs11h_setPINPromptHook(__pinPromptHook, this)) != CKR_OK) {
            throw pkcs11Exception(rv, QStringLiteral("Cannot set hook"));
        }
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::init - return"), Logger::Debug);
}

void pkcs11Provider::deinit()
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::deinit - entry"), Logger::Debug);
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::deinit - return"), Logger::Debug);
}

QString pkcs11Provider::name() const
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::name - entry/return"), Logger::Debug);

    return QStringLiteral("qca-pkcs11");
}

QStringList pkcs11Provider::features() const
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::features - entry/return"), Logger::Debug);
...
Provider::Context *pkcs11Provider::createContext(const QString &type)
{
...
void pkcs11Provider::startSlotEvents()
{
    CK_RV rv;

    QCA_logTextMessage(QStringLiteral("pkcs11Provider::startSlotEvents - entry"), Logger::Debug);

    if (_lowLevelInitialized) {
        if (!_slotEventsLowLevelActive) {
            if ((rv = pkcs11h_setSlotEventHook(__slotEventHook, this)) != CKR_OK) {
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::startSlotEvents - return"), Logger::Debug);
}

void pkcs11Provider::stopSlotEvents()
{
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::stopSlotEvents - entry/return"), Logger::Debug);

    _slotEventsActive = false;
}

QVariantMap pkcs11Provider::defaultConfig() const
{
    QVariantMap mytemplate;

    QCA_logTextMessage(QStringLiteral("pkcs11Provider::defaultConfig - entry/return"), Logger::Debug);
...
void pkcs11Provider::configChanged(const QVariantMap &config)
{
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::configChanged - entry"), Logger::Debug);
    ...
    pkcs11h_setProtectedAuthentication(config[QStringLiteral("allow_protected_authentication")].toBool() != false
                                           ? TRUE
    ...
    QCA_logTextMessage(QStringLiteral("pkcs11Provider::configChanged - return"), Logger::Debug);
}

void pkcs11Provider::__logHook(void *const global_data, const unsigned flags, const char *const format, va_list args)
{
    pkcs11Provider *me = (pkcs11Provider *)global_data;
    me->_logHook(flags, format, args);
}

void pkcs11Provider::__slotEventHook(void *const global_data)
{
    pkcs11Provider *me = (pkcs11Provider *)global_data;
    me->_slotEventHook();
}
...
PKCS11H_BOOL
pkcs11Provider::__tokenPromptHook(void *const              global_data,
                                  void *const              user_data,
                                  const pkcs11h_token_id_t token,
                                  const unsigned           retry)
{
...
PKCS11H_BOOL
pkcs11Provider::__pinPromptHook(void *const              global_data,
                                void *const              user_data,
                                const pkcs11h_token_id_t token,
                                const unsigned           retry,
                                char *const              pin,
...
void pkcs11Provider::_logHook(const unsigned flags, const char *const format, va_list args)
{
...
void pkcs11Provider::_slotEventHook()
{
    /*
...
PKCS11H_BOOL
pkcs11Provider::_tokenPromptHook(void *const user_data, const pkcs11h_token_id_t token)
{
    if (s_keyStoreList != nullptr) {
...
PKCS11H_BOOL
pkcs11Provider::_pinPromptHook(void *const              user_data,
                               const pkcs11h_token_id_t token,
                               char *const              pin,
                               const size_t             pin_max)
{
